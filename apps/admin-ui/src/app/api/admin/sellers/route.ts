...
const getBackendSellers = async (query: string) => {
  for (const base of backendBases) {
    for (const path of [`/admin/sellers${query}`, `/auth/admin/sellers${query}`]) {
      try {
...
export async function GET(request: NextRequest) {
  ...
  try {
    const databaseResult = await getDatabaseSellers(search, page, limit);
    if (databaseResult.sellers.length > 0) {
